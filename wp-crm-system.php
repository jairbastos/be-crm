...
if ( ! defined( 'WP_CRM_SYSTEM_VERSION' ) ) {
  define( 'WP_CRM_SYSTEM_VERSION', '3.2.7' );
}
...
/**
 * Include modal for quick add
 * 
 * Can be re-use if needed
 * 
 * @since	3.2.5
 *
 * We can't use get_current_screen() at this stage
 * 
 * So, we can rely on GET post param instead
 * This means, we are on the edit screen page which is
 * an alternative to the screen
 * 
 * Check the post_type and see if it's valid
 */
$post_id = isset( $_GET['post'] ) ? sanitize_text_field( $_GET['post'] ) : '';
if( ! empty( $post_id ) ) {
	/**
	 * Valid post type we allow for this modal
	 * 
	 * Exit if invalid
	 */
	if ( in_array( get_post_type( $post_id ), WPCRM_POST_TYPES ) ) {
		include_once( WP_CRM_SYSTEM_PLUGIN_DIR . '/includes/wcs-modal-quick-add.php' );
	}
} elseif( isset( $_GET['post_type'] ) ) {

	/**
	 * Ensure that the modal markup won't be visible on the table list page
	 * 
	 * @since	3.2.5.1
	 */
	global $pagenow;
	 ...
	 * Exit if invalid
	 */
	if ( in_array( sanitize_text_field( $_GET['post_type'] ), WPCRM_POST_TYPES ) ) {
		include_once( WP_CRM_SYSTEM_PLUGIN_DIR . '/includes/wcs-modal-quick-add.php' );
	}
}
if( isset( $_GET['page'] ) ) {
	if( 'wpcrm-settings' === esc_html( $_GET['page'] ) ) {
		include( WP_CRM_SYSTEM_PLUGIN_DIR . '/includes/wcs-dashboard-modal-quick-add.php' );
	}
}
